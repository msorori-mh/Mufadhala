...
BEGIN
  ...
  INSERT INTO public.students (
    ...
    phone, governorate, gpa, coordination_number,
    university_id, college_id, major_id
  ) VALUES (
    NEW.id,
    NULLIF(_first_name, ''),
    TRIM(_meta->>'second_name'),
    TRIM(_meta->>'third_name'),
    TRIM(_meta->>'fourth_name'),
    TRIM(_meta->>'phone'),
    TRIM(_meta->>'governorate'),
    CASE WHEN _meta->>'high_school_gpa' IS NOT NULL 
         THEN (_meta->>'high_school_gpa')::numeric 
         ELSE NULL END,
    TRIM(_meta->>'coordination_number'),
    _university_id,
    _college_id,
    _major_id
  );
